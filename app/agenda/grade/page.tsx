          ...
          <div className="flex items-center justify-center px-4 py-3">
            <div className="flex items-center bg-gray-100 rounded-xl p-1 shadow-inner">
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 rounded-lg hover:bg-white hover:text-primary transition-all duration-200"
              ...
              <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="ghost"
                    className="h-9 px-4 rounded-lg hover:bg-white hover:text-primary transition-all duration-200 flex items-center gap-2"
                  >
                ...
                {getCurrentTimePosition !== null && (
                  ...
                  >
                    <div className="flex items-center">
                      <div className="w-2 h-2 rounded-full shadow-lg" style={{ backgroundColor: 'var(--primary)' }}></div>
                      <div className="flex-1 h-0.5 shadow-sm" style={{ backgroundColor: 'var(--primary)' }}></div>
                      <div className="text-white text-[10px] px-1.5 py-0.5 rounded-full shadow-lg font-medium" style={{ backgroundColor: 'var(--primary)' }}>
                    ...
                    <div className="max-h-52 overflow-y-auto">
                      {services.length > 0 ? services.map((service) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center mr-3 flex-shrink-0 ${isSelected ? 'bg-primary border-primary' : 'border-gray-300'}`}>
                              {isSelected && (
                          ...
                          <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                            {['pending', 'confirmed', 'completed', 'canceled'].map((st) => (
                              ...
                              >
                                {getStatusText(st)}
                                {selectedAppointment.status === st && (
                                  <svg className="w-4 h-4 text-primary inline-block ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <DrawerFooter className="pt-4 border-t border-gray-200 space-y-3">
            ...
            {selectedAppointment?.status !== "free" && selectedAppointment?.client?.phone_number && (
              <Button
                ...
                className="w-full h-11 bg-primary hover:bg-[#2d422f] text-white font-medium rounded-lg"
              >
